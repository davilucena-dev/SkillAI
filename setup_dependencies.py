...
"""
    ...
    # ── Painel de processos lateral (tool calls em tempo real) ────────────────
    # Garante que o OpenCode exiba cada etapa do agente no painel esquerdo,
    # sem suprimir tool calls, sem colapsar automaticamente e sem perder eventos.
    cfg["autoscroll"]   = True   # acompanha automaticamente a execução no painel
    cfg["toolDetails"]  = True   # expande os detalhes de cada tool call no painel

    # Keybind para abrir/fechar o painel lateral (padrão: ctrl+p).
    # Definir explicitamente evita conflito com atalhos do Colab/ttyd.
    cfg.setdefault("keybinds", {})
    cfg["keybinds"]["toggle_process_panel"] = "ctrl+shift+p"

    # Garante que o painel de processos inicie aberto por padrão
    cfg.setdefault("layout", {})
    cfg["layout"]["process_panel"] = "open"

    # Não limita o histórico de tool calls exibidos no painel
    cfg["maxToolCallsInPanel"] = 0   # 0 = sem limite

    with open(OPENCODE_CFG, "w") as f:
        json.dump(cfg, f, indent=2)

    print("✅ Agente SkillAI configurado:", agent_path)
    print("✅ Painel de processos ativado no config:", OPENCODE_CFG)
